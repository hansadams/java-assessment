...
public class CourseService {
    ...
    public Map<String, Double> studentGrades(String courseId) {
        ...
        if ( students != null ) {
            ...
            for (Student student: students){
                double grade = student.getCourseGrades().get(courseId);
    ...
    public double averageGrades(String courseId) {

        Double total = 0d;

        if ( studentGrades(courseId) != null ) {

            for (Map.Entry<String, Double> entry : studentGrades(courseId).entrySet()) {
                Double value = entry.getValue();

                total += value;
            }

            return total / studentGrades(courseId).size();
    ...
    public void printGrades(String courseId) {
        ...
        if ( studentGrades(courseId) != null ) {
            ...
            for (Map.Entry<String, Double> entry : studentGrades(courseId).entrySet()) {
                ...
                System.out.println("Student{" + "name=" + key + ", grade=" + value + '}');
                System.out.println("Average Grade: " + this.averageGrades(courseId));
